    ...
    if  request.method == 'POST':
        ...
        if (form.is_valid()):
            ...
            if is_right and not TaskContestCase.objects.filter(task=task).exists():
                ...
                newsol = Solution(username=request.user, answer=ans, description=description, verdict=Virdict.ACCEPTED_FOR_EVUALETION_IN_CONTEST, task=task, need_rang=rang, comments=[])
            else:
